...
const Skills = () => {
  const skillCategories = [
    {
      ...
      icon: <Layout />,
      skills: ['HTML', 'CSS', 'React', 'TailwindCSS' , 'JavaScript']
    },
    {
  ...
  return (
    <section id="skills" className="py-20 bg-[#F9F9F9] pl-5 pr-5">
      <motion.div
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {skillCategories.map((category, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="bg-white p-6 rounded-lg hover:bg-[#F9F9F9]-800/70 transition-colors"
            >
              ...
              <ul className="space-y-2">
                {category.skills.map((skill) => (
                  <li key={skill} className="text-black-500">
                    • {skill}
                  </li>
                ))}
              </ul>
